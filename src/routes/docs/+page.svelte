	...
	<main class="mx-auto max-w-4xl px-4 py-6 sm:px-6">
		...
		>
			{#if activeDoc === 'integration'}
				...
				<p>
					Dokumen ini menjelaskan alur transmisi data dari tingkat perangkat keras (Sensor) hingga
					tertampil secara <em>real-time</em> di aplikasi web pemantauan. Arsitektur Tomatica
					menggunakan <strong>Supabase</strong> (PostgreSQL) sebagai jembatan data utama antara
					<em>Edge Node</em> (Raspberry Pi/ESP32) dan Aplikasi Web.
				</p>

				<h3>1. Topologi Alur Data (Data Flow)</h3>
				<p>
					Sistem ini mengikuti alur komunikasi satu arah dari perangkat pengirim menuju <em
						>database</em
					>, dan <em>event-driven</em> dari <em>database</em> menuju aplikasi web.
				</p>
				<p>
					<code
				...
				<p>
					Skrip Python <code>monitoring_tomat.py</code> pada Raspberry Pi secara terus-menerus membaca
				...
				<ul>
					...
					<li>
						Seluruh data mentah dikalkulasi menjadi nilai absolut yang bermakna (misal: mm/h, lux,
						%).
					</li>
				</ul>
				...
				<p>
					Pada setiap akhir siklus <em>interval</em> (standarnya 60 detik), skrip Python akan
					menyatukan data menjadi objek JSON. Data ini dikirimkan menuju <em>REST API</em> milik Supabase
				...
				<p>
					<em
						>Autentikasi HTTP menggunakan JWT Token (Service Role / Anon Key) disematkan dalam
						header permintaan untuk menjamin keamanan pengiriman.</em
					>
				</p>

				<h3>2. Bagaimana Aplikasi Web Menerima Data?</h3>
				<p>
					Aplikasi <em>dashboard</em> dibangun menggunakan kerangka kerja <strong>SvelteKit</strong>
					yang bekerja secara sangat reaktif. Tidak ada server perantara tambahan, aplikasi ini
				...
				<ul>
					<li>
						Aplikasi melakukan kueri SQL melalui Supabase JS Client untuk meminta 50 baris riwayat
						data terakhir.
					</li>
					<li>
						Data tersebut diolah seketika menjadi grafik (Chart.js) dan daftar tabel. Baris ke-0
						(terbaru) dikirimkan ke modul <em>Dashboard</em>.
					</li>
				</ul>

				<h4>B. Sinkronisasi Real-Time (WebSockets)</h4>
				<p>
					Alih-alih melakukan <em>refresh</em>, aplikasi Web membuka jalur komunikasi WebSocket
					menggunakan layanan <code>Supabase Realtime</code>.
				</p>
				<pre><code
						>// Contoh Cuplikan Kode Integrasi di +page.svelte
supabase
    ...
    .subscribe();</code
					></pre>
				<p>
					Ketika Raspberry Pi berhasil menyuntikkan baris JSON baru ke tabel, mekanisme <em
						>Postgres Triggers</em
					>
					di Supabase langsung mengirimkan <em>broadcast</em> ke WebSocket. Hasilnya: Kartu di
					<em>dashboard</em> Anda berkedip hijau, dan angka seketika berubah tanpa perlu memuat ulang
					peramban.
				</p>

				<h3>3. Deteksi Perangkat Aktif/Terputus (IsOnline)</h3>
				<p>
					Selain menampilkan data, aplikasi <em>dashboard</em> juga memeriksa aktivitas pengiriman:
				</p>
				<ul>
					<li>
						Setiap kali <em>event</em> INSERT masuk, aplikasi mencatat
						<strong>stempel waktu (timestamp)</strong>.
					</li>
					<li>Sistem memiliki penghitung mundur statis di latar belakang (3 menit / 180 detik).</li>
					<li>
						Jika waktu saat ini (<code>Date.now()</code>) melampaui stempel waktu masuknya data
						terakhir sejauh 3 menit, maka indikator konektivitas akan berubah menjadi "Terputus"
			...
			{:else if activeDoc === 'rpi'}
				...
				<p>
					Dokumen ini menjelaskan cara kerja, konfigurasi, dan penggunaan skrip Python <code
						>monitoring_tomat.py</code
					> yang dijalankan pada Raspberry Pi.
				</p>

				<h3>1. Pendahuluan</h3>
				<p>
					Skrip <code>monitoring_tomat.py</code> dirancang untuk menjadikan Raspberry Pi sebagai
					<strong>Edge Gateway</strong>
					sekaligus pembaca sensor. Berbeda dengan ESP32 yang menggunakan Arduino, Raspberry Pi
					membaca sensor menggunakan protokol I2C dan GPIO secara langsung, lalu mencatatnya secara
					lokal dan mengirimkannya ke database <em>cloud</em> (Supabase).
				</p>
				...
				<ul>
					<li>
						<strong><code>INTERVAL</code></strong>: Jeda pembacaan dan pengiriman data (dalam
				...
				<ol>
					<li>
						<strong>Inisialisasi:</strong> Menyiapkan library, mengatur GPIO BCM, serta memulai komunikasi
						I2C dan 1-Wire.
					</li>
					<li><strong>Pembuatan File CSV:</strong> Membuat baris judul jika belum ada.</li>
					<li>
						<strong>Looping Monitoring:</strong>
						<ul>
							<li>Menghitung pulsa sensor hujan dan angin selama durasi interval.</li>
							<li>
								Setelah interval berlalu, skrip membaca nilai suhu, kelembaban, cahaya, dan tanah.
							</li>
				...
				<p>
					Program akan menampilkan log pembacaan sensor dan status pengirimannya ke Supabase.
					Disarankan menggunakan <em>background service</em> seperti <code>systemd</code>.
				</p>
			{:else}
				<h2>Dokumentasi ESP32 (Tomatica)</h2>
				<p>
					Dokumen ini menjelaskan cara kerja, konfigurasi, dan penggunaan kode firmware <code
						>tomatica.ino</code
				...
				<h3>1. Pendahuluan</h3>
				<p>
					Kode <code>tomatica.ino</code> dirancang untuk menjadikan ESP32 sebagai
					<strong>Sensor Node</strong>
					yang bertugas membaca berbagai sensor lingkungan dan tanah secara langsung. Data yang
					dibaca kemudian ditampilkan melalui <em>Serial Monitor</em> dan perangkat mendukung pembaruan
					program tanpa kabel (Over-The-Air / OTA).
				</p>
				...
				<ul>
					<li>
						...
						K. Terhubung via <code>Serial2</code>.
					</li>
				...
				<p>
					...
					<strong>ArduinoOTA</strong>
					aktif dengan nama <em>ESP32-Weather-Station-Pro</em>. Ini memungkinkan Anda mengunggah
					program (.bin) via WiFi tanpa kabel USB.
				</p>

				<h3>5. Alur Kerja Program (Cara Kerja)</h3>
				<ol>
					<li>
						<strong>Setup Awal:</strong> Inisialisasi Serial, I2C, sensor, pin interrupt, dan menyambung
						ke WiFi.
					</li>
					<li>
						<strong>Looping Utama:</strong>
						<ul>
					...
					<li>
						<strong>Pembacaan Sensor:</strong> Membaca data I2C, 1-Wire, dan RS485, lalu menghitung kecepatan
						angin dan curah hujan (yang direset per jam/hari).
					</li>
					<li>
						<strong>Pencetakan Data:</strong> Menampilkan hasil pembacaan ke Serial Monitor pada baud
						rate 115200.
					</li>
				</ol>
