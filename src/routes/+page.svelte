	...
	<main class="mx-auto max-w-6xl px-4 py-8 sm:px-6">
		...
		<div class="mb-8">
			<h1 class="text-2xl font-bold text-gray-900 sm:text-3xl">
				{activeView === 'dashboard' ? 'Dashboard Monitoring' : activeView === 'stats' ? 'Statistik Sensor' : 'Tabel Data Sensor'}
			</h1>
		...
		{:else if activeView === 'stats'}
			...
			{:else if tableRows.length === 0}
				<div class="flex flex-col items-center justify-center rounded-2xl border border-dashed border-gray-200 bg-white py-20">
					<div class="mb-4 rounded-full bg-gray-50 p-4 text-gray-400"><Database class="h-8 w-8" /></div>
					<h3 class="text-lg font-semibold text-gray-900">Data belum tersedia</h3>
					<p class="mt-1 max-w-xs text-center text-sm text-gray-500">Pastikan perangkat IoT Anda terhubung dan mengirimkan data.</p>
				</div>
			{:else}
				<div class="flex flex-col gap-8">
					{#each chartGroups as group (group.id)}
						<div>
							...
							<div class="mb-3 flex items-center gap-2.5">
								<span class="text-base leading-none" aria-hidden="true">{group.icon}</span>
								<h2 class="text-xs font-bold uppercase tracking-widest text-gray-400">{group.label}</h2>
								<div class="h-px flex-1 bg-gray-100"></div>
							</div>
								...
								{#each group.series as serie (serie.key)}
									...
									<div class="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
										...
										<div class="mb-4 flex items-start justify-between">
											<div>
												<p class="text-xs font-semibold uppercase tracking-wider text-gray-400">{serie.label}</p>
												<div class="mt-1 flex items-baseline gap-1">
													<span class="text-3xl font-bold tracking-tight text-gray-900">{latest ?? '-'}</span>
													<span class="text-sm font-semibold" style="color:{serie.color}">{serie.unit}</span>
												</div>
											</div>
											{#if maxVal != null}
												<div class="flex gap-4 text-right">
													<div>
														...
														<p class="text-sm font-bold" style="color:{serie.color}">{maxVal} {serie.unit}</p>
													</div>
													<div>
										...
										<!-- Canvas chart -->
										{#if points.length < 2}
											<div class="flex h-44 items-center justify-center rounded-lg bg-gray-50 text-xs text-gray-300">
												Data tidak cukup untuk menampilkan grafik
											</div>
										{:else}
											<div class="relative h-44">
												<canvas
													use:chartAction={{ points, serie }}
					...
					{#each tableRows as row (row.id)}
						<div class="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm">
							...
							<div class="px-4 pb-3 pt-3">
								<p class="mb-2 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-sky-600">
							...
							<!-- Group: Kondisi Tanah -->
							<div class="px-4 pb-3 pt-3">
								<p class="mb-2 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-emerald-600">
								...
								<div class="grid grid-cols-2 gap-x-4 gap-y-2">
									...
									<div class="flex items-center justify-between">
										...
										<span class="text-sm font-bold text-cyan-600">{row.phosphorus ?? '-'} mg/kg</span>
									</div>
									<div class="flex items-center justify-between">
							...
							<!-- Group: Cuaca & Cahaya -->
							<div class="px-4 pb-3 pt-3">
								<p class="mb-2 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-indigo-600">
						...
						<table class="w-full text-sm">
							<thead>
								...
								<tr class="border-b border-gray-100 bg-gray-50/50">
									...
									<th
										colspan="4"
										class="border-x border-sky-100 bg-sky-50/60 px-4 py-2 text-center text-xs font-bold uppercase tracking-wider text-sky-600"
									>
										🌤️ Kondisi Udara
									</th>
									<!-- Kondisi Tanah -->
									<th
										colspan="5"
										class="border-x border-emerald-100 bg-emerald-50/60 px-4 py-2 text-center text-xs font-bold uppercase tracking-wider text-emerald-600"
									>
										🌱 Kondisi Tanah
									</th>
									<!-- Cuaca -->
									<th
										colspan="3"
										class="border-l border-indigo-100 bg-indigo-50/60 px-4 py-2 text-center text-xs font-bold uppercase tracking-wider text-indigo-600"
									>
										🌦️ Cuaca & Cahaya
									</th>
								</tr>
								...
								<tr class="border-b border-gray-100 bg-gray-50">
									<th class="px-4 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-red-400">Suhu (°C)</th>
									<th class="px-4 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-sky-400">Kelembapan (%)</th>
									<th class="px-4 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-violet-400">CO₂ (ppm)</th>
									<th class="border-r border-gray-100 px-4 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-orange-400">AQI</th>
									<th class="px-4 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-emerald-400">Kelembapan (%)</th>
									<th class="px-4 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-lime-500">pH</th>
									<th class="px-4 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-green-500">N (mg/kg)</th>
									<th class="px-4 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-cyan-500">P (mg/kg)</th>
									<th class="border-r border-gray-100 px-4 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-teal-500">K (mg/kg)</th>
									<th class="px-4 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-amber-400">Cahaya (Lux)</th>
									<th class="px-4 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-indigo-400">Hujan (mm/h)</th>
									<th class="px-4 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-teal-400">Angin (m/s)</th>
								</tr>
							</thead>
							<tbody>
								{#each tableRows as row (row.id)}
									<tr class="border-b border-gray-50 transition-colors duration-150 hover:bg-red-50/30">
										<td class="px-4 py-3 font-medium text-gray-500 whitespace-nowrap">{formatTime(row.created_at)}</td>
										<!-- Udara -->
										...
										<td class="px-4 py-3 text-right font-semibold text-violet-500">{row.co2 ?? '-'}</td>
										<td class="border-r border-gray-100 px-4 py-3 text-right font-semibold text-orange-500">{row.aqi ?? '-'}</td>
										<!-- Tanah -->
										<td class="px-4 py-3 text-right font-semibold text-emerald-500">{row.soil_moisture ?? '-'}</td>
										<td class="px-4 py-3 text-right font-semibold text-lime-600">{row.soil_ph ?? '-'}</td>
										<td class="px-4 py-3 text-right font-semibold text-green-600">{row.nitrogen ?? '-'}</td>
										<td class="px-4 py-3 text-right font-semibold text-cyan-600">{row.phosphorus ?? '-'}</td>
										<td class="border-r border-gray-100 px-4 py-3 text-right font-semibold text-teal-600">{row.potassium ?? '-'}</td>
										<!-- Cuaca -->
